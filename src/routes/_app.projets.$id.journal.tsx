...
import {
  formatDate, formatEUR, decisionStatusLabel,
  type DecisionStatus, type Priority, type Note,
} from "@/lib/mockData";
...
const priorityOrder: Record<Priority, number> = {
  critique: 0, haute: 1, moyenne: 2, basse: 3,
};
    ...
    <div className="space-y-6">
      ...
      {mainTab === "decisions" && (
        <div className="space-y-4">
          <Tabs value={decisionFilter} onValueChange={(v) => setDecisionFilter(v as typeof decisionFilter)}>
            <TabsList className="flex h-auto w-full flex-wrap justify-start gap-1 bg-secondary/60 p-1">
              <TabsTrigger value="all" className="text-xs">Toutes ({decisions.length})</TabsTrigger>
              <TabsTrigger value="a_trancher" className="text-xs">À trancher ({counts.a_trancher})</TabsTrigger>
              <TabsTrigger value="validee" className="text-xs">Validées ({counts.validee})</TabsTrigger>
              ...
              <CardContent className="space-y-3 p-4 sm:space-y-4 sm:p-5">
                ...
                <ul className="space-y-1">
                  {d.options.map((opt) => {
                    ...
                    return (
                      <li key={opt} className={cn("flex items-start gap-2 rounded-md px-2 py-1 text-sm", isSelected ? "bg-success/10 font-medium text-success" : "text-muted-foreground")}>
                        <CheckCircle2 className={cn("mt-0.5 h-3.5 w-3.5 shrink-0", isSelected ? "text-success" : "opacity-0")} />
                        <span>{opt}</span>
                      </li>
                    );
                  })}
      ...
      {mainTab === "notes" && (
        <div className="space-y-3">
          {sortedNotes.length === 0 && <p className="py-12 text-center text-sm text-muted-foreground">Aucune note pour l'instant.</p>}
          {sortedNotes.map((n) => {
            ...
            return (
              <Card key={n.id} className="border-border/60 shadow-sm">
                <CardContent className="flex items-start gap-3 p-4">
                  <div className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-md", n.type === "decision" && "bg-success/15 text-success", n.type === "note" && "bg-info/15 text-info", n.type === "alerte" && "bg-destructive/15 text-destructive")}>

...
function BudgetPage() {
  ...
  const rows: Row[] = projectLots.map((l) => {
    ...
    if (l.realCost !== null) {
      ...
      if (diff > l.budgetPlanned * 0.05) { status = "Écart"; tone = "warning"; }
      else { status = "Dans le budget"; tone = "info"; }
    }
    return { name: l.name, planned: l.budgetPlanned, quoteMin: min, quoteMax: max, real: l.realCost, status, tone };
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Cible" value={formatEUR(project.budgetTarget)} icon={<Wallet className="h-4 w-4" />} />
        <StatCard label="Prévu (avec marge)" value={formatEUR(totalPlanned + RESERVE)} hint={`Marge imprévus : ${formatEUR(RESERVE)}`} />
        <StatCard label="Engagé" value={formatEUR(stats.engaged)} icon={<TrendingUp className="h-4 w-4" />} tone="info" />
        <StatCard label="Reste" value={formatEUR(project.budgetTarget - stats.engaged)} icon={<TrendingDown className="h-4 w-4" />} tone={project.budgetTarget - stats.engaged < 0 ? "danger" : "success"} />
      </div>
            ...
            <Table>
                ...
                {rows.map((r, i) => {
                  ...
                  return (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-right tabular-nums">{formatEUR(r.planned)}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatEUR(r.quoteMin)}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatEUR(r.quoteMax)}</TableCell>
                      <TableCell className="text-right tabular-nums">{formatEUR(r.real)}</TableCell>
                      <TableCell className={cn("text-right tabular-nums", diff && diff > 0 ? "text-destructive" : diff && diff < 0 ? "text-success" : "text-muted-foreground")}>
              ...
              <TableFooter>
                <TableRow>
                  ...
                  <TableCell className="text-right font-semibold tabular-nums">{formatEUR(totalPlanned)}</TableCell>
                  <TableCell className="text-right tabular-nums">{formatEUR(totalQuoteMin)}</TableCell>
                  <TableCell />
        ...
        <CardContent className="flex items-start gap-3 p-4">
          ...
          <div className="text-sm">
            ...
            <p className="text-muted-foreground">
              Le devis maçonnerie dépasse de 400 € le budget prévu. Pensez à arbitrer avant d'engager d'autres lots critiques.
            </p>
          </div>
        </CardContent>
      </Card>
